...
class LUS_Database {
    ...
    private function get_recording_path($user_id) {
        return LUS_Constants::UPLOAD_DIR . '/' . date('Y/m') . '/' . $user_id;
    }
    ...
    // TEMP METHOD to just get the machinery going on install
    /**
     ...
     */
    public function get_recordings($args = []) {
        $defaults = [
            'user_id' => 0,
            'passage_id' => 0,
            'status' => '',
     ...
     */
    public function get_all_assignments($args = []) {
        global $wpdb;
        ...
        // Build the main query with user and passage information
        $query = "SELECT a.*,
                  u.display_name AS user_name,
                  p.title AS passage_title
                  FROM {$wpdb->prefix}lus_assignments a
                  LEFT JOIN {$wpdb->users} u ON a.user_id = u.ID
                  LEFT JOIN {$wpdb->prefix}lus_passages p ON a.passage_id = p.id";
        ...
        // Add ORDER BY clause - make sure we have valid values
        $orderby = in_array($args['orderby'], ['assigned_at', 'due_date', 'status']) ? $args['orderby'] : 'assigned_at';
        ...
        // Only use prepare if we have arguments to escape
        if (!empty($where_args)) {
    ...
    public function get_passage($passage_id) {
        ...
        // Here's where we're getting the prepare warning
    ...
    public function get_all_passages($args = []) {
        ...
        if (!empty($where_args)) {
            $query = $this->db->prepare(
                "SELECT * FROM {$this->db->prefix}lus_passages
                WHERE " . implode(' AND ', $where) . "
                ORDER BY " . sanitize_sql_orderby($args['orderby'] . ' ' . $args['order']) .
                $limit_clause . $offset_clause,
                $where_args
            );
        } else {
            $query = "SELECT * FROM {$this->db->prefix}lus_passages
                    WHERE " . implode(' AND ', $where) . "
                    ORDER BY " . sanitize_sql_orderby($args['orderby'] . ' ' . $args['order']) .
                    $limit_clause . $offset_clause;
        }
    ...
    public function assign_passage_to_user($passage_id, $user_id, $assigned_by, $due_date = null) {
        try {
            ...
            $result = $this->db->insert(
                $this->db->prefix . 'lus_assignments',
                $insert_data,
                [
                    '%d', // passage_id
                    '%d', // user_id
                    ...
                    '%s', // assigned_at
                    '%s', // status
                    $due_date ? '%s' : null // due_date (if provided)
                ]
            );
    ...
    /**
     * Get unassigned recordings with user details
     */
    public function get_unassigned_recordings($limit = 20, $offset = 0) {
        $sql = $this->db->prepare(
            ...
            LIMIT %d OFFSET %d",
            array($limit, $offset)  // Parameters must be in an array
        );
    ...
    public function get_total_unassigned_recordings() {
        // Since this query has no placeholders, don't use prepare()
        return (int)$this->db->get_var(
            "SELECT COUNT(*)
            FROM {$this->db->prefix}lus_recordings
